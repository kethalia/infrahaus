...
<task type="auto">
  ...
  <action>
Extend the existing `src/lib/containers/actions.ts` (Phase 3 created it with `createContainerAction`). ADD the new actions below the existing createContainerAction.

**Important: Use `authActionClient` pattern** — per CLAUDE.md conventions and existing codebase patterns, ALL server actions must use `authActionClient` from `@/lib/safe-action`. This matches the existing `createContainerAction`, `discoverTemplatesAction`, `deleteTemplateAction`, etc. Define Zod input schemas for each action (simple `z.object({ containerId: z.string() })`) and use `authActionClient.schema(schema).action(async ({ parsedInput }) => { ... })`.

Import `authActionClient` and `ActionError` from `@/lib/safe-action`. Use `throw new ActionError("message")` for user-facing errors (handled by safe-action's `handleServerError`). Do NOT define a custom `ActionResult` type — use next-safe-action's built-in return format (`{ data }` on success, `{ serverError }` on failure via ActionError).

Define a shared input schema:

```ts
const containerIdSchema = z.object({ containerId: z.string() });
```
...
2. If lock not acquired, `throw new ActionError("Another operation is already in progress on this container")`
3. Wrap the entire action body in a `try/finally` that calls `releaseContainerLock(containerId)` in the `finally` block

**Create these server actions using authActionClient:**

1. **`startContainerAction`**

   ```ts
   export const startContainerAction = authActionClient
     .schema(containerIdSchema)
     .action(async ({ parsedInput: { containerId } }) => {
       // acquire lock, try/finally pattern...
     });
   ```

   - Acquire lock: `acquireContainerLock(containerId, "start")` — throw ActionError if lock not acquired
   - `try {` — all remaining steps in try block
   - Call `createProxmoxClientFromSession()` to get client + node
   - Call `DatabaseService.getContainerById(containerId)` — throw ActionError if not found
   - Call `getContainer(client, node.name, container.vmid)` to check Proxmox status — throw ActionError if already running
   ...
   - Call revalidatePath, return `{ success: true }`
   - On error: `throw new ActionError("Failed to start container")` (generic message, no Proxmox detail leakage)
   - `} finally { await releaseContainerLock(containerId); }`

2. **`stopContainerAction`** — same `authActionClient.schema(containerIdSchema).action(...)` pattern
   - Same lock pattern: acquire lock → try/finally with releaseContainerLock
   - Get client, get container, check status (throw ActionError if already stopped)
   - Call `stopContainer(client, node.name, container.vmid)` — force stop
   - Wait for task (30s timeout)
   - Create "stopped" event
   - Return `{ success: true }`

3. **`shutdownContainerAction`** — same authActionClient pattern
   - Same lock pattern: acquire lock → try/finally with releaseContainerLock
   - Get client, get container, check status (throw ActionError if not running)
...
4. **`restartContainerAction`** — same authActionClient pattern
   - Same lock pattern: acquire lock → try/finally with releaseContainerLock
...
5. **`deleteContainerAction`** — same authActionClient pattern
   - Same lock pattern: acquire lock → try/finally with releaseContainerLock
   - Get client, get container (throw ActionError if not found)
   ...
   - Return `{ success: true }`
   - On error: `throw new ActionError("Failed to delete container")`

**Patterns to follow:**

- Use `authActionClient.schema(schema).action()` for all actions (matches existing codebase patterns per CLAUDE.md)
- Use `throw new ActionError(msg)` for user-facing errors (handled by safe-action's error pipeline)
...
**Revalidation:** After each successful action, call `revalidatePath("/")` and `revalidatePath("/containers/[id]")` (use the actual format `revalidatePath("/containers/" + containerId)`) to trigger Next.js cache refresh. Import from `next/cache`.
  ...
  <verify>
    ...
    - All 5 actions use `authActionClient.schema(containerIdSchema).action()` pattern (matching existing createContainerAction and CLAUDE.md conventions)
    ...
    - Each action throws `ActionError` when lock not acquired or on user-facing errors
  ...
  <done>
    - 5 lifecycle server actions using authActionClient pattern: start, stop, shutdown (with fallback), restart, delete
    ...
    - Uses ActionError for user-facing errors, no Proxmox detail leakage
  </done>
</task>
