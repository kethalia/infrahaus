...
must_haves:
  truths:
    - "run-scripts.sh can SSH into a running VM and execute all post-install scripts in order"
    - "README documents the full workflow: create VM, wait for cloud-init, optionally run scripts manually"
    - "The template has a complete usage path from zero to working desktop VM"
  artifacts:
...
Purpose: Complete the template by providing (1) a way to run post-install scripts on an existing VM (useful when cloud-init isn't enough or for re-configuration), and (2) documentation that explains the full workflow.
...
- Cloud-init didn't complete fully (timeout, package failures)
- User wants to re-run setup after making changes to scripts
- User created a VM manually without cloud-init
...
2. **Overview** — What this template creates (Debian 13 VM with XFCE desktop for OpenClaw)
3. **Requirements** — Proxmox VE 8+, internet access, storage with enough space
4. **Quick Start** — The two commands:
   - `bash create-vm.sh` (creates VM with cloud-init)
   - Wait for cloud-init to complete (~15-20 min)
