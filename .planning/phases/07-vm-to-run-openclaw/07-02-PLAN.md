...
must_haves:
  truths:
    - "Each script is idempotent — safe to re-run without breaking existing state"
    - "Scripts cover all locked requirements: desktop, auto-login, Chrome, Node.js, VNC, QEMU agent, OpenClaw"
    - "Scripts follow the numbered ordering convention from the LXC template pattern"
    - "Scripts can run standalone inside a Debian 13 VM as root"
  artifacts:
...
<objective>
Create the numbered post-install scripts that run inside the VM to configure the desktop environment and install all required software. These scripts serve as an alternative/supplement to cloud-init — they can be run manually via SSH if cloud-init fails or for re-configuration.
...
<task type="auto">
  <name>Task 2: Create application and service scripts (03-06, 99)</name>
  <files>
    infra/vm/templates/openclaw-desktop/scripts/03-nodejs-setup.sh
    infra/vm/templates/openclaw-desktop/scripts/04-chrome-install.sh
    infra/vm/templates/openclaw-desktop/scripts/05-vnc-setup.sh
...
<verify> - All five scripts pass syntax check: `for f in infra/vm/templates/openclaw-desktop/scripts/{03,04,05,06,99}-*.sh; do bash -n "$f"; done` - All five scripts are executable - 03 references nodesource setup_22.x - 04 uses modern GPG keyring (not apt-key) - 05 creates vncserver systemd service - 06 installs openclaw via npm - 99 validates all components
</verify>
<done> - 03-nodejs-setup.sh installs Node.js 22 via NodeSource - 04-chrome-install.sh installs Chrome with modern GPG keyring approach - 05-vnc-setup.sh configures TigerVNC with systemd service on port 5901 - 06-openclaw-install.sh installs OpenClaw globally via npm - 99-post-setup.sh validates everything and prints welcome message with access details - All scripts are idempotent and can run standalone
</done>
</task>
