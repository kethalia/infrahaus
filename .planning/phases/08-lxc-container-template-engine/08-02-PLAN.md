...
must_haves:
  truths:
    ...
    - "Pipeline order: validate → pre-deploy hook → pct create → start → packages → files → scripts → post-deploy hook → summary"
  artifacts:
