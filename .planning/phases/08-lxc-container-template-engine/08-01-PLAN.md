...
must_haves:
  truths:
    ...
    - "Hook execution runs pre/post scripts if they exist"
  artifacts:
...
- `run_hook "$template_dir" "$hook_name" "$ctid" "$username"` — runs a hook script if it exists. Hook names: `pre_deploy`, `post_deploy`. Looks for `$template_dir/hooks/pre-deploy.sh` or `$template_dir/hooks/post-deploy.sh` (maps underscore to hyphen in filename). The hook script is pushed into the container at `/tmp/hook-$hook_name.sh`, executed, then removed.
- Hooks receive environment variables: `CTID`, `HOSTNAME`, `USERNAME`, `TEMPLATE_DIR`
- If hook file doesn't exist, log_debug and skip (not an error).
- If hook fails (non-zero exit), log*error and return 1 (caller decides whether to abort).
  </action>
  <verify>
  ...
  <done>Three library files exist with no bash syntax errors. container.sh has 10+ ct* functions, files.sh handles USER replacement in paths and contents, hooks.sh runs hook scripts inside container.</done>
  </task>
