...
Since this repo already tracks Prisma migrations under `apps/dashboard/prisma/migrations/`, keep schema history consistent by adding a proper migration rather than using `prisma db push --force-reset`.

Run `npx prisma migrate dev --name infra-refactor` to apply the schema changes (the migration will drop the `rootPassword` column and add `userId` — add `userId` as nullable first, then backfill or alter to required). Then run `npx prisma generate` to regenerate the Prisma client.

If a full reset of a purely local dev database is needed and data loss is acceptable, use `npx prisma migrate reset` (which still respects the recorded migrations) instead of `prisma db push --force-reset`.
  </action>
  <verify>
