...
<task type="auto">
  <name>Task 2: Migrate containers/actions.ts, containers/data.ts, and containers/schemas.ts</name>
  <files>
    apps/dashboard/src/lib/containers/actions.ts
    apps/dashboard/src/lib/containers/data.ts
    apps/dashboard/src/lib/containers/schemas.ts
  ...
  <action>
This is the bulk migration — 8 call sites across actions.ts and data.ts, plus schema cleanup.
...
All actions now receive `ctx.userId` from authActionClient (Plan 02). Use this to resolve nodes.
...
2. **`getWizardData()` (line ~204)** — Currently calls `getProxmoxClient()`. Refactor to accept `nodeId: string` parameter. Resolve the node from DB: `DatabaseService.getNodeById(nodeId)`, then `createProxmoxClientFromNode(node)`. Remove `process.env.PVE_NODE` reference — get the node name from the DB record (`node.name`). If no nodeId provided, return early with a "no node" indicator so the wizard can show the graceful degradation banner. This function also needs to be aware that it may be called from a context where the user has no nodes — handle gracefully.
...
5. **`refreshContainerServicesAction` (line ~881)** — Currently calls `getProxmoxClient()` AND uses `PVE_HOST`/`PVE_ROOT_PASSWORD` for SSH. Refactor the Proxmox client part: resolve from container's node. The SSH part will be migrated in Plan 05 — for now, leave a TODO comment for the SSH portion OR refactor SSH too since we have the node's sshPassword. Actually, this action creates an SSH connection using env vars. Refactor to: get the node from the container, use `node.host` and `decrypt(node.sshPassword)` for SSH credentials. If `node.sshPassword` is null, throw ActionError("SSH password not configured for this node. Update node settings.").

6. **`getContainersWithStatus()` in data.ts (line ~153)** — Currently calls `getProxmoxClient()`. Refactor: this function lists ALL containers from ALL nodes for the dashboard. Change to: accept `userId: string` parameter. Get all user's nodes via `DatabaseService.listNodesForUser(userId)`. For each node, create a client and fetch containers in parallel. Merge results.

7. **`getContainerDetailData()` in data.ts (line ~274)** — Currently calls `getProxmoxClient()`. Refactor: the container record has a `node` relation. Use `createProxmoxClientFromNode(container.node)`.

**Remove all imports of `getProxmoxClient` from both files.** Add imports for `createProxmoxClientFromNode`, `DatabaseService`, `decrypt` as needed.

**Remove ALL `process.env.PVE_HOST`, `process.env.PVE_PORT`, `process.env.PVE_ROOT_PASSWORD`, `process.env.PVE_NODE` references.**

**Important considerations:**
- `getContainersWithStatus` signature changes — callers (page.tsx) will need to pass userId. The page.tsx will get userId from session in its RSC. This is a breaking change that's expected — Plan 08 (dashboard updates) will fix the page.
- For now, if type errors cascade to page files, that's OK. Focus on making actions.ts and data.ts internally consistent.
  </action>
  <verify>
Run `grep -r "getProxmoxClient\b" apps/dashboard/src/lib/containers/` — should return 0 results.
Run `grep -r "PVE_HOST\|PVE_ROOT_PASSWORD\|PVE_PORT\|PVE_NODE" apps/dashboard/src/lib/containers/` — should return 0 results.
Run `grep -r "rootPassword" apps/dashboard/src/lib/containers/` — should return 0 results.
...
Read containers/data.ts and verify it uses createProxmoxClientFromNode.
  ...
  <done>
All 8 call sites in containers/ migrated from env-var to DB-based auth. getOrCreateNode() deleted. rootPassword removed from schemas and creation flow. No env-var references remain. getContainersWithStatus accepts userId for multi-node queries.
  </done>
</task>
