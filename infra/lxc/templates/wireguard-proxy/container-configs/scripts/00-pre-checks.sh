#!/usr/bin/env bash
# 00-pre-checks.sh — Verify system state and log environment info (WireGuard Proxy)
#
# This script runs first and validates that the container environment is
# ready for WireGuard Proxy setup. It logs critical system information and checks for
...
# Check memory (minimum 512 MB required for WireGuard Proxy)
TOTAL_MEM_KB=$(grep MemTotal /proc/meminfo | awk '{print $2}')
TOTAL_MEM_MB=$((TOTAL_MEM_KB / 1024))
log_info "  Total Memory: ${TOTAL_MEM_MB} MB"

if [[ $TOTAL_MEM_MB -lt 512 ]]; then
    log_warn "  Memory is below recommended 512 MB. Some operations may be slow."
fi
...
if [[ $DISK_AVAIL_GB -lt 5 ]]; then
    log_warn "  Disk space is below recommended 5GB free."
fi
...
if [[ $CPU_CORES -lt 1 ]]; then
    log_warn "  CPU cores below recommended 1. Consider allocating more cores."
fi
...
log_info "System is ready for WireGuard Proxy configuration."
