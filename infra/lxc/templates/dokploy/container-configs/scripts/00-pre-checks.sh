#!/usr/bin/env bash
# 00-pre-checks.sh — Verify system state and log environment info (Dokploy)
#
# This script runs first and validates that the container environment is
# ready for Dokploy setup. It logs critical system information and checks for
...
# Check memory (minimum 2048 MB required for Dokploy)
TOTAL_MEM_KB=$(grep MemTotal /proc/meminfo | awk '{print $2}')
TOTAL_MEM_MB=$((TOTAL_MEM_KB / 1024))
log_info "  Total Memory: ${TOTAL_MEM_MB} MB"

if [[ $TOTAL_MEM_MB -lt 2048 ]]; then
    log_warn "  Memory is below recommended 2048 MB. Some operations may be slow."
fi
...
log_info "System is ready for Dokploy configuration."
